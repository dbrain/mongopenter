...
Db.prototype.createShards = function (cb) {
  ...
  var shardTags = this.shards ? this.shards.shardTags : undefined;
  if (shardTags) {
    console.log('Mongopenter: Creating shards...');
    var jobs = [];
    var self = this;
    this.getConnection(function (err, db) {
      shardTags.forEach(function (shardAddress) {
        jobs.push(self._createShard.bind(self, db, shardAddress.shard, shardAddress.host));
      });
      async.series(jobs, function (err, results) {
        db.close();
        cb(err, results);
...
Db.prototype._createShard = function(db, shardName, shardAddress, cb) {
  console.log(" - Creating shard: " + shardAddress);

  db.command({ listShards: 1}, function (err, result) {
    if (err) {
      cb(err, null);
    }
    var shardAlreadyExists = false; 

    result.shards.forEach(function (existingShard) {
      if (existingShard.host === shardAddress) {
        shardAlreadyExists = true;
      }
    });
    if (!shardAlreadyExists) {
      db.command({ addshard:shardAddress, name: shardName }, function(err, result) {
        if (result.errmsg) return cb(result.errmsg);
        cb(null, result);
      });
    } else {
      console.log("Shard: " + shardAddress + " already exists - skipping creation task");
      cb(null, "skipped shard creation");
    } 
  });
};

Db.prototype.addShards = function (cb) {
  if (this.shards && this.setup.shards && this.shards.shardTags) {
    console.log('Mongopenter: Adding shards tags...');
    var self = this;
    var shardKey = self.shards.shardKey;
    var shardTags = self.shards.shardTags;
    ...
    this.getConnection(function (err, db) {
      shardTags.forEach(function (shardTag) {
        jobs.push(self._addShardTag.bind(self, db, shardKey, shardTag));
      });
      jobs.push(self._enableSharding.bind(self, db, shardDb));
      jobs.push(self._shardCollection.bind(self, db, shardCollection, shardKey));

      jobs.push(self._addTagRanges.bind(self, db, shardCollection, self.shards.shardKey));
...
Db.prototype._addShardTag = function(db, shardKey, shardTag, cb) {
  console.log(" - Adding tag: " + shardKey + " to shard: " + shardTag.shard);
  ...
  config.collection("shards", function (err, configCollection) {
    ...
    configCollection.findOne( { _id : shardTag.shard }, function (err, item) {
      ...
      configCollection.update( { _id: shardTag.shard }, { $addToSet: { tags: shardKey } }, function (err, item) {
        if (err) return cb(err);
        cb(undefined, item);
      });
    });
