...
Db.prototype.createShards = function (cb) {
  ...
  var shardAddresses = this.shards.hosts;
  if (shardAddresses) {
    var jobs = [];
    var self = this;
    this.getConnection(function (err, db) {
...
Db.prototype._createShard = function(db, shardAddress, cb) { 
  console.log("adding shard: " + shardAddress);
...
Db.prototype.addShards = function (cb) {
  ...
  var shardTags = this.setup.shards.shardTags;
  var shardDb = this.setup.shards.shardDb;
  var shardCollection = this.setup.shards.shardCollection;
  var shardKey = this.setup.shards.shardKey;
  var hostnames = Object.keys(this.shards.keyMap);  
  if (shardTags) {
    var jobs = [];
    var self = this;
    this.getConnection(function (err, db) {
      ...
      hostnames.forEach(function(host) {
        ...
        jobs.push(self._addTagRanges.bind(self, db, shardCollection, {"shardKey" : tag}, {"shardKey": tag + "z"}, tag));
      });
...
Db.prototype._addShardTag = function(db, shardTag, cb) { 
  console.log("adding to shard: " + shardTag.shard + " tag: " + shardTag.tag);
...
Db.prototype._addTagRanges = function(db, ns, min, max, tag, cb) {
  console.log("adding tag ranges: " + JSON.stringify(min) + " / " + JSON.stringify(max) + " to tag: " + tag);
  var config = db.db("config");
  config.collection("tags", function (err, tagsCollection) {
    ...
    tagsCollection.update( {_id: { ns : ns , min : min } }, { $set: { ns : ns , min : min , max : max , tag : tag }},  { upsert: true }, 
      function (err, item) {
        console.log("update the tags collection: " + err + " result: " + JSON.stringify(item));
        if (err) {
          cb(err);
        } else {
          cb(undefined, item);
